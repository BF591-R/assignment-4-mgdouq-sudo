...
caption_4 <- 'Figure 1: Proportion of variance explained by each principal component (bar) and cumulative variance explained (line) across all 35 PCs derived from gene expression data of 35 samples.'
caption_5 <- 'Figure 2: Biplot of PC1 vs PC2 scores for 35 samples colored by SixSubTypesClassification, illustrating the separation of breast cancer subtypes captured by the top two principal components which together explain 24.1% of total variance.'
caption_8 <- 'Figure 3: Heatmap of normalized intensity values for differentially expressed probes (FDR < 0.01) across all samples, where rows represent significant probes and columns represent samples.'
```
...
variance_tibble$principal_components <- factor(variance_tibble$principal_components, levels = variance_tibble$principal_components)
